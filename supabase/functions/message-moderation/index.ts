...
serve(async (req: Request) => {
  ...
  try {
    ...
    const flagged = analysisResults.filter((r: any) => r.risk_score >= 30);
    if (flagged.length > 0) {
      const riskLogs = flagged.map((r: any) => ({
      ...
      if (flagged.some((r: any) => r.risk_score >= 60)) {
        const criticalFlags = flagged.filter((r: any) => r.risk_score >= 60);
        await supabase.from("ai_intelligence_logs").insert({
          ...
          details: criticalFlags.map((r: any) => `[${r.risk_category}] Score: ${r.risk_score} - ${r.details}`).join("\n"),
          source_table: "messages",
        });
      }
    }

    return new Response(JSON.stringify({ success: true, scanned: unscanned.length, flagged: flagged.length, high_risk: flagged.filter((r: any) => r.risk_score >= 60).length }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
