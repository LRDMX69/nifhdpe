...
serve(async (req) => {
  ...
  try {
    ...
    const lowStock = (inventory.data ?? []).filter(i => i.quantity_meters !== null && i.min_stock_level !== null && i.quantity_meters < i.min_stock_level);

    const prompt = `Generate a daily executive summary for NIF Technical operations:\n\nACTIVE PROJECTS: ${projects.data?.length ?? 0}\nLOW STOCK ITEMS: ${lowStock.length}\nRECENT EXPENSES TOTAL: ₦${(expenses.data ?? []).reduce((s, e) => s + Number(e.amount), 0).toLocaleString()}\nRECENT PAYMENTS TOTAL: ₦${(payments.data ?? []).reduce((s, p) => s + Number(p.amount), 0).toLocaleString()}\nFIELD REPORTS TODAY: ${reports.data?.length ?? 0}\nEQUIPMENT COUNT: ${equipment.data?.length ?? 0}\nATTENDANCE TODAY: ${attendance.data?.length ?? 0}\n\nProvide:\n1. Executive Summary\n2. Critical Alerts\n3. Risk Assessment\n4. Key Metrics\n5. Recommended Actions\n\nBe concise, strategic, data-driven. Use ₦.`;
