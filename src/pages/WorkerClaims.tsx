...
=======
    ...
    onSuccess: () => {
      ...
      setOpen(false); setCategory(""); setAmount(""); setDescription(""); setProofFile(null);
>>>>>>> Stashed changes
...
=======
                  ...
                  <Input type="file" accept="image/*" onChange={(e) => setProofFile(e.target.files?.[0] || null)} className="cursor-pointer" />
                  {proofFile && <p className="text-[10px] text-primary truncate">Selected: {proofFile.name}</p>}
                </div>
                <Button className="w-full" onClick={() => submitClaim.mutate()} disabled={!category || submitClaim.isPending || uploading}>
...
<<<<<<< Updated upstream
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Pending/Flagged</p><p className="text-2xl font-bold text-warning">{pendingCount}</p></CardContent></Card>
        <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Total Claims</p><p className="text-2xl font-bold">{claims.length}</p></CardContent></Card>
        <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Approved Total</p><p className="text-2xl font-bold text-primary">{formatCurrency(totalAmount)}</p></CardContent></Card>
=======
...
=======
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <div className="space-y-1 flex-1 min-w-0">
                    ...
                    {c.proof_url && (
                      <div className="mt-2 relative group w-32 h-32 rounded-md overflow-hidden border border-border bg-muted/50">
                        <img 
                          src={c.proof_url} 
                          alt="Proof" 
...
>>>>>>> Stashed changes
                  ...
                  {hasImage && (
                    <div className="rounded-lg overflow-hidden border border-border bg-muted/30">
                      <img src={c.file_url} alt="Claim proof" className="w-full max-h-48 object-contain" loading="lazy" />
                    </div>
                  )}
                  {c.file_url && !hasImage && (
                    <div className="rounded-lg border border-border bg-muted/30 p-3 text-center">
                      <Button size="sm" variant="outline" asChild>
                        <a href={c.file_url} target="_blank" rel="noopener noreferrer">📄 Open Attached Document</a>
                      </Button>
                    </div>
                  )}
