...
export const InvoiceDialog = ({ open, onOpenChange, onCreated }: Props) => {
  ...
  const canSubmit = Boolean(clientId) && items.every((item) => item.description.trim() && item.quantity > 0 && item.unit_price >= 0) && subtotal > 0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[92dvh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2 justify-end"><Button variant="outline" onClick={() => onOpenChange(false)} disabled={saving}>Cancel</Button><Button onClick={handleSubmit} disabled={saving || !canSubmit}>{saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Create invoice atomically</Button></div>
        </div>
      </DialogContent>
    </Dialog>
  );
