...
export function HRFinanceWorkspace({ orgId, userId, members, profileMap, activeRole }: WorkspaceProps) {
  ...
  const [salary, setSalary] = useState(blank({ employee: "", start: today().slice(0, 8) + "01", end: today(), bank: "none", accountNumber: "", gross: "", pension: "", voluntary: "", tax: "", deductions: "", loan: "" }));
  const [overtime, setOvertime] = useState(blank({ employee: "", month: today().slice(0, 7) + "-01", gross: "", workingDays: "", days: "", bank: "none", accountNumber: "" }));
  const [loan, setLoan] = useState(blank({ employee: "", amount: "", additional: "", months: "", start: today(), bank: "none", notes: "" }));
  const [hmo, setHmo] = useState(blank({ employee: "", start: today(), end: "", classification: "individual", amount: "" }));
  const [vat, setVat] = useState(blank({ date: today(), clientName: "", tin: "", state: "", lga: "", gross: "", outputVat: "", inputVat: "", withholding: "", note: "" }));
  ...
  const salaryNet = Math.max(0, Number(salary.gross || 0) - Number(salary.pension || 0) - Number(salary.voluntary || 0) - Number(salary.tax || 0) - Number(salary.deductions || 0) - Number(salary.loan || 0));
  const saveSalary = () => saveRecord.mutate({ table: "hr_salary_schedules", payload: { employee_id: salary.employee, period_start: salary.start, period_end: salary.end, bank_account_id: salary.bank === "none" ? null : salary.bank, account_number: salary.accountNumber || null, gross_salary: Number(salary.gross || 0), pension: Number(salary.pension || 0), voluntary_contribution: Number(salary.voluntary || 0), tax: Number(salary.tax || 0), deductions: Number(salary.deductions || 0), loan_repayment: Number(salary.loan || 0), net_pay: salaryNet, status: "submitted", submitted_by: userId } }, { onSuccess: () => { toast({ title: "Salary schedule submitted" }); setSalaryOpen(false); refresh(["hr-salary-schedules"]); } });
