...
export const SplashScreen = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    ...
    const done = setTimeout(onComplete, 1100);
    return () => { clearTimeout(timer); clearTimeout(done); };
  }, [onComplete]);

  return (
    <div
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-background transition-opacity duration-300 ${fadeOut ? "opacity-0 pointer-events-none" : "opacity-100"}`}
    >
      <img
        src={nifLogo}
        alt="NIF Technical"
